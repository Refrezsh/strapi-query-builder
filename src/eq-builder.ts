...
> {
   ...
   * @param {EQBuilderCallback} builderFactory Fabric function that returns builder with filters for relation model
   */
  ...
  >(
    ...
    return this as unknown as EQBuilder<
      ...
      Data,
      {
        ...
        populates: {
          [P in keyof Config["populates"] | K]: P extends K
            ? BuildEQCallback<RelationConfig>
            : P extends keyof Config["populates"]
            ? Config["populates"][P]
            : never;
        };
    ...
    populates.forEach((populate) => {
      ...
      if (dynamicQuery) {
        ...
        for (const key of Object.keys(dynamicQuery)) {
          parsedDynamicZone[key] = EQBuilder._buildQuery(dynamicQuery[key]);
        }
...
type BuildEQCallback<Config extends EntityBuilderConfig> = {
  ...
  filters: ParseFilters<
    ...
    Config["negate"]
  >;
  populate: ParseEQBuilderPopulates<Config["populates"], Config["populateAll"]>;
